...
def controladorAnfis(model, tstep, SETPOINT, n_horizon, n_robust, inputPenalty): 
    ...
    mterm = 100*((model.x['h_anfis']-SETPOINT)**2) # terminal cost
    lterm = 1000*((model.x['h_anfis']-SETPOINT)**2) # stage cost
    mpc.set_objective(mterm=mterm, lterm=lterm)
    mpc.set_rterm(u_anfis = inputPenalty) # input penalty
...
mpc = controladorAnfis(modelAnfis, tstep, 3, n_horizon, n_robust, input_penalty)
...
for k in range(tsimu):
    u0 = mpc.make_step(X0)
    u_mpc[0][k] = u0
    #np.savetxt('INPUT_Planta_' + ensaios_label[i] + '.txt', u_mpc[0])
    np.savetxt('INPUT_Planta_' + "Res_2" + '.txt', u_mpc[0])

    y = plantaSimu(float(u0[0][0]), float(volumeA))
    HsimuPlanta[0][k] = y[2]
    volumeA = y[1]
    #np.savetxt('OUTPUT_Planta_' + ensaios_label[i] + '.txt', HsimuPlanta[0])
    np.savetxt('OUTPUT_Planta_' + "Res_2" + '.txt', HsimuPlanta[0])
    ...
    np.savetxt('ANFIS_Planta_' + "Res_2" + '.txt', HsimuAnfis_h[0])

    Einit[0][k] = HsimuPlanta[0][k]-HsimuAnfis[0][0]
    #np.savetxt('ERRO_Planta_' + ensaios_label[i] + '.txt', Einit[0])
    np.savetxt('ERRO_Planta_' + "Res_2" + '.txt', Einit[0])

    X0 = np.array([HsimuPlanta[0][k], u0[0][0]])

    if k == 300:
        #mpc = controladorAnfis(modelAnfis, tstep, 5, n_horizon[i], n_robust[i], input_penalty[i])
        mpc = controladorAnfis(modelAnfis, tstep, 5, n_horizon, n_robust, input_penalty)
        mpc.x0 = X0
        mpc.set_initial_guess()
    """ elif k == 600:
        #mpc = controladorAnfis(modelAnfis, tstep, 4, n_horizon[i], n_robust[i], input_penalty[i])
        mpc = controladorAnfis(modelAnfis, tstep, 4, n_horizon, n_robust, input_penalty)
        mpc.x0 = X0
        mpc.set_initial_guess()
    elif k == 900:
        #mpc = controladorAnfis(modelAnfis, tstep, 6, n_horizon[i], n_robust[i], input_penalty[i])
        mpc = controladorAnfis(modelAnfis, tstep, 6, n_horizon, n_robust, input_penalty)
        mpc.x0 = X0
        mpc.set_initial_guess()
    elif k == 1200:
        #mpc = controladorAnfis(modelAnfis, tstep, 2, n_horizon[i], n_robust[i], input_penalty[i])
        mpc = controladorAnfis(modelAnfis, tstep, 2, n_horizon, n_robust, input_penalty)
        mpc.x0 = X0
        mpc.set_initial_guess()
